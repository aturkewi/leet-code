...
class Solver
  attr_reader :queries, :int_length

  def initialize(queries, int_length)
    @queries = queries
  ...
  def starting_number
    @starting_number ||= set_starting_number
  end

  def set_starting_number
    zero_count = (int_length - 1) / 2
    starting_number = 1 * 10 ** zero_count
  end
  
  def generate_number(first_half)
    if int_length == 1
        first_half
